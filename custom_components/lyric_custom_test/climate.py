...
LYRIC_HVAC_MODES = {
    ...
    # Do not include HVACMode.AUX_HEAT, handle via preset mode
}
...
class LyricClimate(LyricDeviceEntity, ClimateEntity):
    ...
    def __init__(
        ...
        # Do not add HVACMode.AUX_HEAT, handle via preset mode
    ...
    def preset_mode(self) -> str | None:
        ...
        lyric_mode = self.device.changeable_values.mode
        if lyric_mode == LYRIC_HVAC_MODE_EMERGENCY_HEAT:
            return PRESET_EMERGENCY_HEAT
    ...
    async def async_set_preset_mode(self, preset_mode: str) -> None:
        ...
        try:
            if preset_mode == PRESET_EMERGENCY_HEAT:
                # Set the HVAC mode to EmergencyHeat in the Lyric API
                await self._update_thermostat(
                    self.location,
                    self.device,
                ...
                # Handle other preset modes as before
                await self._update_thermostat(
                    self.location,
                    self.device,
